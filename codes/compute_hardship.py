"""
This is the module file to train and predict on hardship data.
...
def prediction_step(background_train, background_test, hardship_data, challengeID_train):
	...
	hardship_data_np = hardship_data.as_matrix()
	hardship_data_np = np.asmatrix(hardship_data_np)
	hardship_data_np = np.ravel(hardship_data_np)
	...
	method.fit(background_train_np, hardship_data_np)
	predict_hardship = method.predict(background_test_np)
	filename = 'predict_hardship_'+method_label+'.csv'
	if os.path.isfile(filename) :
		os.remove(filename)
	for i in range(len(predict_hardship)):
...
def hardship_calculation(path, train_data, background_data, challengeID_train):

	print('We are computing hardship')
	...
	"""
	hardship_data = train_data[train_data.columns[3]].copy()
	hardship_data = hardship_data.fillna(hardship_data.mean())
	...
	prediction_step(background_train, background_test, hardship_data, challengeID_train)

	print 'hardship Runtime:', str(time.time() - start_time)


if __name__ == '__main__':
	print('This is the module file for calculating the hardships.\n\
